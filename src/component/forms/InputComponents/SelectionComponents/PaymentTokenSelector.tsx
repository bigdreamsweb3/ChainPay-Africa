    ...
    return (
        <>
            <div className="max-w-md mx-auto">
                <motion.div
                    className="bg-white rounded-xl overflow-hidden shadow-lg border border-chainpay-blue-light/20"
                        ...
                        <div className="space-y-3">
                            <div className="flex flex-row items-stretch gap-3">
                                ...
                                <div className="flex-1 relative">
                                    <label htmlFor="amount" className="block text-xs font-medium text-chainpay-blue-dark/70 mb-1.5 ml-0.5">
                                        Amount (min. 50)
                                    </label>
                                ...
                                <div className="relative" ref={dropdownRef}>
                                    <label htmlFor="tokenSelect" className="block text-xs font-medium text-chainpay-blue-dark/70 mb-1.5 ml-0.5">
                                        Payment Token
                                    </label>
                                    <motion.button
                                        id="tokenSelect"
            ...
            <AnimatePresence>
                {selectedTokenData && creditAmount && !isNaN(Number(creditAmount)) && Number(creditAmount) >= 50 && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: -5 }}
                        className="mt-6 mb-5 px-4 py-3 rounded-lg bg-gradient-to-r from-chainpay-orange-light/20 to-chainpay-orange/15 border border-chainpay-orange-light/40 shadow-sm"
                    >
                        <div className="flex items-center justify-between">
                            <p className="text-sm flex items-center gap-2">
                                <CreditCard className="w-4 h-4 text-chainpay-orange drop-shadow-sm" />
                                <span className="font-medium text-gray-700">Estimated payment:</span>
                                {isConverting ? (
                                    <span className="flex items-center">
                                        <Loader2 className="w-3.5 h-3.5 mr-1.5 animate-spin text-chainpay-orange" />
                                        <span className="text-chainpay-orange-dark">Calculating...</span>
                                    </span>
                                ) : (
                                    <span className="text-gray-900 font-semibold">
                                        {localDisplayAmount} <span className="text-chainpay-orange-dark">{selectedTokenData?.symbol}</span>
                                    </span>
                                )}
                            </p>
                        </div>
                        {conversionError && (
                            <p className="text-xs text-chainpay-orange-dark mt-1.5 flex items-center gap-1">
                                    ...
                                    {paymentTokens.map((token: PaymentToken) => (
                                        ...
                                        >
                                            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-chainpay-blue-light/20 to-chainpay-blue/30 flex items-center justify-center overflow-hidden border border-chainpay-blue-light/50 shadow-sm">
                                                <Image
                                                    src={getTokenImageSource(token.network, token.token) || "/placeholder.svg"}
